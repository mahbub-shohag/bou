...
foreach ( $items as $row ){
    ...
    if( $row['subjects_name'] != "" ){
        ...
        // echo "<pre>"; print_r($course);print_r($student);exit;
        //$courses[] = $course;
        if(1==1)//$student['learners_id']==$previous_learners_id)
        {
            array_push($student['course'],$course);
        }
        else{
            foreach ($students as $one_student)
            {
                if($one_student['learners_id'] == $student['course'])
                {
                    array_push($one_student['course'], $course);
                }
            }
        }
        $previous_student = $student['learners_id'];
        echo "<pre>"; print_r($course);print_r($student);exit;
