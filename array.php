...
LEFT JOIN study_centers ON learners.current_study_centers_id = study_centers.study_centers_id limit 100";
//echo $query;exit;
//$items = find($query);
$result = query($query);
$items = array();
...
$is_first_time = 1;
//print_r($items);exit;
$previous_student=array();
foreach ( $items as $row ){
    //print_r($row);exit;
    $course = array();    
    if(!in_array($row['learners_id'], $student)){
        ...
        if(1==1)//$row['learners_id']==$previous_student['learners_id'])
        {
